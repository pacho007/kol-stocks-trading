  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Portfolio · Your Trader Holdings | SHARPS" },
      {
        ...
        content: "Holdings, entry price, live value and P&L in one book.",
      },
    ],
  }),
  component: Portfolio,
...
function Portfolio() {
  ...
  const rows = positions
    .map((p) => {
      ...
      // entry (cost basis) isn't derivable from an on-chain balance alone —
      // null until a fills-indexer backs this. P&L is unknown, not zero, in
      // that case, so it's left out of the aggregate rather than faked.
      const cost = p.entry != null ? p.entry * p.shares : null;
      const pnl = cost != null ? value - cost : null;
      const pnlPct = cost != null && cost !== 0 ? (pnl! / cost) * 100 : null;
      return { kol, ...p, price, value, cost, pnl, pnlPct };
    })
    ...
    <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6">
      ...
      {hasUnknownCostBasis && (
        <p className="mt-2 text-[11px] text-muted-foreground">
          Some positions have an unknown entry price (cost basis isn&apos;t derivable from an
          on-chain balance alone) — P&amp;L above excludes them rather than assuming zero.
        </p>
      )}
          ...
          <table className="w-full min-w-[820px]">
            <thead>
              <tr className="border-b border-border bg-surface/60 text-left">
                {["Trader", "Shares", "Avg entry", "Price", "Value", "P&L", "Return", ""].map(
                  (h) => (
                    <th
                      key={h}
            ...
            <tbody>
              {rows.map((r) => (
                ...
                >
                  ...
                  <td
                    className={`num px-4 py-3 text-sm ${r.pnl == null ? "text-muted-foreground" : r.pnl >= 0 ? "text-up" : "text-down"}`}
                  >
                    {r.pnl != null ? fmtUsd(r.pnl) : "—"}
                  </td>
                  <td
                    className={`num px-4 py-3 text-sm ${r.pnlPct == null ? "text-muted-foreground" : r.pnlPct >= 0 ? "text-up" : "text-down"}`}
                  >
